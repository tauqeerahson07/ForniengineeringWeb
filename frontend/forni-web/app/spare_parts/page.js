...
const SpareParts = () => {
  const { spareParts, loading, error } = useContext(DataContext);
  const [searchQuery, setSearchQuery] = useState('');
  const [sortBy, setSortBy] = useState('name');
  const router = useRouter();

  // Filter and sort spare-parts
  const filteredAndSortedSpareParts = React.useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      filtered = spareParts.filter(part =>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="text-center">
            <div className="bg-red-50 border border-red-200 rounded-lg p-6">
              <h2 className="text-xl font-semibold text-red-800 mb-2">Error Loading Spare Parts</h2>
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50 pt-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-lg text-gray-600 max-w-2xl mx-auto">
            Discover our comprehensive range of high-quality industrial spare parts designed for various applications and industries.
          </p>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-8">
          <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
            ...
            <div className="text-sm text-gray-600">
              {filteredAndSortedSpareParts.length} spare part{filteredAndSortedSpareParts.length !== 1 ? 's' : ''} found
            </div>
          </div>
        </div>
          ...
          <div className="text-center py-12">
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-8">
              ...
              <p className="text-gray-600">
                {searchQuery.trim()
                  ? `No spare parts match your search "${searchQuery}"`
      ...
      <div className="relative h-60 bg-gray-100">
        ...
        {/* View Details Link */}
        <Link
          href={`/spare-parts/${encodeURIComponent(sparePart.name)}`}
      ...
      <div className="p-4">
        ...
        <div className="space-y-2 mb-4">
          {sparePart.category && (
            <div className="flex items-center text-sm">
              ...
              <span className="ml-2 text-gray-600">{sparePart.category}</span>
            </div>
          )}
        ...
        <div className="flex flex-col gap-2">
          <button
            onClick={() => handleGetQuotation(sparePart.id, sparePart.name)}
        ...
        <div className="mt-12 bg-orange-50 rounded-lg p-8 text-center">
          <h2 className="text-2xl font-bold text-gray-900 mb-4">
            Need a Custom Spare Part?
          </h2>
          <p className="text-gray-600 mb-6 max-w-2xl mx-auto">
            Can't find exactly what you're looking for? Our team of experts can design and manufacture custom spare-parts tailored to your specific requirements.
          </p>
          <Link
