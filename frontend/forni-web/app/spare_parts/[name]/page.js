...
const SparePartsDetailPage = () => {
  ...
  const sparePart = data.getSparePartByName(decodedName);
  ...
  const generateImageList = () => {
    const images = [];
    if (sparePart) {
      if (sparePart.cover_image) {
        images.push({ image: sparePart.cover_image, alt: `${sparePart.name} - Cover Image` });
      }
      if (sparePart.gallery_images && sparePart.gallery_images.length > 0) {
        sparePart.gallery_images.forEach((img, index) => {
          images.push({ image: img, alt: `${sparePart.name} - Gallery Image ${index + 1}` });
        });
      }
    }
    return images;
  ...
  useEffect(() => {
    ...
    setCurrentImageIndex(0); // Reset to first image when spare part changes
  }, [sparePart]);
      ...
      <div className="min-h-screen bg-gray-50 pt-20">
        <div className="max-w-7xl mx-auto px-4 py-8 text-center">
          ...
          <p className="text-gray-600 mb-8">
            The spare part {decodedName} could not be found.
          </p>
          <button
  ...
  const handleGetQuotation = () => {
    const subject = `Quotation Request - ${sparePart.name}`;
